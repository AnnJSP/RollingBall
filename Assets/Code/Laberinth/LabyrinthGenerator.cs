...
namespace Code
{
    public class LabyrinthGenerator : MonoBehaviour, IController
    {
        ...
        public LabyrinthGenerator(Transform parent, Data data)
        {
            _parent = parent;
            _labyrinthSizeX = data.LabyrinthSizeX;
            _labyrinthSizeZ = data.LabyrinthSizeZ;
            _cellPrefub = Resources.Load<GameObject>("Plane");
            _wallPrefub = Resources.Load<GameObject>("Wall");
        ...
        public void GenerateLabyrinth()
        {
            ...
            do
            {
                ...
                if (CheckCell(rowX + 1, columnZ) && CheckPlaceAroundCell(rowX + 1, columnZ, Direction.Right))
                    cellsForNextStep.Add(new Cell(rowX + 1, columnZ, 
                        PositionSetter.Set(position, _cellGridSizeX, _cellGridSizeZ, Direction.Right)));

                // left
                if (CheckCell(rowX - 1, columnZ) && CheckPlaceAroundCell(rowX - 1, columnZ, Direction.Left))
                    cellsForNextStep.Add(new Cell(rowX - 1, columnZ, 
                        PositionSetter.Set(position, _cellGridSizeX, _cellGridSizeZ, Direction.Left)));

                // up
                if (CheckCell(rowX, columnZ + 1) && CheckPlaceAroundCell(rowX, columnZ + 1, Direction.Up))
                    cellsForNextStep.Add(new Cell(rowX, columnZ + 1, 
                        PositionSetter.Set(position, _cellGridSizeX, _cellGridSizeZ, Direction.Up)));

                // down
                if (CheckCell(rowX, columnZ - 1) && CheckPlaceAroundCell(rowX, columnZ - 1, Direction.Down))
                    cellsForNextStep.Add(new Cell(rowX, columnZ - 1, 
                        PositionSetter.Set(position, _cellGridSizeX, _cellGridSizeZ, Direction.Down)));
